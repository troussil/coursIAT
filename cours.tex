...
\begin{frame}
  ...
  \only<3->{
    ...
    \item $f$ peut être quasiment plat près du min (instabilité numérique) 
...
\begin{frame}
  ...
  \begin{block}{Théorème}
    ...
    \item $\forall x$, le hessien $\nabla^2f(x)$ est une matrice semi-définie positive, c-à-d. ses valeures propres sont positives.
    \end{enumerate}
  \end{block}

  \begin{block}{Note:}
    Les valeurs propres d'une matrice carré sont les composantes du vecteur
