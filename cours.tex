...
\end{frame}
    ...
    %% Les composantes de $x$, c-à-d. $x_1, \dots, x_n$, peut être supposées dans $\{0,1\}$,
    %% car on peu toujous passer de variables entières à des variables binaires en nombre suffisant) 
...
\begin{frame}
  ...
  \begin{itemize}
    \item En rajoutant des variables, il est possible de se restreindre à des variables entières positives %2 variables >= 0, pour une variable libre
    \item En maximisant la somme des coordonnées (sous les mêmes contraintes), il est possible de borner supérieurement les variables,
      car l'ensemble des solutions est suppposé borné%on cherche à maximiser la sum des coord
    \item Il y a donc un nombre \emph{fini} de combinaisons de variables à énumérer, mais ce nombre est en pratique tellement grand, qu'il n'est pas envisageable de faire une \emph{énumération exhaustive}
      %c-à-d. que même pour de relativement petits problèmes, le temps que mettrait un ordinateur pour énumérer toutes les possibilités est de l'ordre de milliers d'années
  \end{itemize}
