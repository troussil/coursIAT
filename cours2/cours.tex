...
\begin{frame}
  ...
  \only<1>{
    \begin{algorithm}[H]
      \caption{backtracking(pb, sol, var, val, seuil)}
      \label{alg:backtracking}
      \If{estPossible(pb, sol, var, val, seuil)}{
        ...
        \Else{
          ...
          (flag, res) $\leftarrow$ backtracking(pb, sol, varS, val, seuil) \;
          \lIf{flag}{
            \Return (flag, res)
          }
 ...
 %%  \caption{backtracking(pb, sol, var, val, seuil)}
 %%  \label{alg:backtracking}
 %%  \KwOut{paire (booleen, affectation)}
 %%  \tcp{poursuivre avec les valeurs suivantes \dots}
 %%  valS $\leftarrow$ valSuivante(pb, val) \;
 %%  \While{estValide(pb, valS)}{
 %%    (flag, res) $\leftarrow$ backtracking(pb, sol, varS, val, seuil) \;
...
\begin{frame}
  ...
  \only<2>{
    ...
    \item On peut noter les inconnues et contraintes $x^T = (x_1,x_2,x_3,x_4)$,
      ...
      ($x_i$ contient le numéro de tâche affectée à l'individu $i$).
      
    \item Pour exprimer la fonction objectif, on peut écrire pour tout $i$
      $x_i := \sum_{j=1}^4 j d_{i,j} = d_{i,1} + 2d_{i,2} + 3d_{i,3} + 4d_{i,4} $ \\
      ($d_{i,j} = 1$ si la tâche $j$ est affectée à l'individu $i$, $d_{i,j} = 0$ sinon).
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item choix de l'individu selon lequel on sépare : arbitraire %(variable) 
